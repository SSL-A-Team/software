	...
	<v-container class="d-flex flex-column">
		<v-container class="d-flex flex-column">
			<v-card
				...
				v-for="robot of this.state.world.teams[this.state.world.team].robots"
				ref="robotCard"
				style="outline-offset: -1px"
				@click.stop="this.state.setJoystickRobot(robot.id)"
			>
				{{ robot.id }}
...
export default {
	...
	methods: {
				...
				if (robot.isValid()) {
					...
					switch (errorLevel) {
						...
						case ErrorLevel.Critical:
							element.animate([{ background: 'red', outline: 'solid 5px red' }], {
		...
		drawStatus: function (robot: Robot, ctx: CanvasRenderingContext2D) {
			...
			ctx.beginPath();
			ctx.arc(0, 0, 0.025 * scale, 0, 2 * Math.PI);
			ctx.closePath();
			ctx.fill();

			const pos = {
				x: [-0.055, 0.055, -0.035, 0.035],
				y: [-0.035, -0.035, 0.055, 0.055]
			};
			...
			for (var i = 0; i < 4; i++) {
				...
				ctx.arc(pos.x[i] * scale, pos.y[i] * scale, 0.025 * scale, 0, 2 * Math.PI);
				ctx.closePath();
				ctx.fill();
			}
			...
			if (robot.status.breakbeam_ball_detected) {
				...
				ctx.beginPath();
				ctx.arc(0, -0.09 * scale, 0.009 * 2.5 * scale, 0, 2 * Math.PI);
				ctx.closePath();
				ctx.fill();
			}
			...
			const wheels = {
				startX: [-0.075, 0.075, -0.035, 0.035, -0.05],
				startY: [-0.095, -0.095, 0.12, 0.12, -0.09],
				endX: [-0.12, 0.12, -0.1, 0.1, 0.05],
				endY: [-0.03, -0.03, 0.07, 0.07, -0.09],
				textX: [-0.12, 0.12, -0.12, 0.12, 0],
				textY: [-0.12, -0.12, 0.14, 0.14, -0.13]
			};
			...
			if (!robot.status.radio_connected) {
				...
				ctx.textAlign = 'center';
				ctx.textBaseline = 'middle';
				ctx.fillText('DC', 0.14 * scale, -0.14 * scale);
			}
		}
	},
	computed: {
