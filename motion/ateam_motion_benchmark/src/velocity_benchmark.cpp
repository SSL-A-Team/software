...
public:
  ...
  : Node("velocity_benchmark"), options_(options)
  {
    ...
    create_indexed_subscribers<ateam_msgs::msg::VisionStateRobot>(
      robot_state_subs_, state_topic_prefix, 1, &VelocityBenchmarkNode::VisionStateRobotCallback,
      this);
...
private:
  ...
  void RunBenchmark()
  {
    ...
    const auto firmware_speed = options_.axis == Axis::X ?
      robot.motion_feedback->body_control_telemetry.kf_body_vel_estimate[0] :
      robot.motion_feedback->body_control_telemetry.kf_body_vel_estimate[1];
