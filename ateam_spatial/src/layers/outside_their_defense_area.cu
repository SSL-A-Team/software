...
CUDA_HOSTDEV float OutsideTheirDefenseArea(const float x, const float y, const FieldDimensions & field_dims, const SpatialSettings & settings) {
  ...
  const auto neg_x_thresh = (field_dims.field_length / 2.0f) - (field_dims.defense_area_depth + (robot_radius * 1.1f));
  const auto pos_y_thresh = (field_dims.defense_area_width / 2.0f) + (robot_radius * 1.1f);
